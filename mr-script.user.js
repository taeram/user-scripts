...
// server variable lets you be logged on to different servers with different
...
// animation-start event which then does whatever needs doing with the new
// elements. in our case, the AJAX 'Results:' boxes are always starting with
...
// TODO:
// we need to check multiple <b> nodes here, for cases where there is, for
// example, multi-stage crafting.
...
function equip(o) {
  ...
  if (o.oname !== undefined) {
    ie += "&outfitname=" + o.oname;
  }
  if (o.onum !== undefined) {
    ie += "&whichoutfit=" + o.onum;
  }
  if (o.i !== undefined) {
    ie += "&whichitem=" + o.i;
  }
...
// Set/GetCharData: store/retrieve data related to a particular
// account/ascension
...
function FindMaxQuantity(item, howMany, deefault, safeLevel) {
  ...
  if (hp == 1) {
    result = integer(GetCharData("maxHP") - GetCharData("currentHP"));
  }
  else {
    result = integer(GetCharData("maxMP") - GetCharData("currentMP"));
  }
  if (result == 0) {
    return 0;
  }
...
function AppendBuyBox(itemNumber, whichStore, buttonText, noQuantityBox) {
  ...
  var quantityString;
  if (noQuantityBox == 1) {
    quantityString = "hidden";
  }
  else {
    quantityString = "text";
  }
  if (prefAutoclear == 2) {
    eventString = ' onFocus="this.select();"' +
        'onClick="this.select();" OnDblClick="this.select();"';
  }
  else if (prefAutoclear == 1) {
    eventString =
...
// causes clicking on a number to fill that number in to the first "quantity"
...
function AppendOutfitSwap(outfitNumber, text) {
  ...
  if (outfitNumber == 0) {
    GM_get(server + "account_manageoutfits.php", function (response) {
      ...
      if (index != -1) {
        index = response.indexOf('name=delete', index) + 11;
      }
      if (index != -1) {
        index2 = response.indexOf('>', index);
      }
      if (index != -1 && index2 != -1) {
...
function InvChecker(event) {
  ...
  } // correct for prices in the mall
    //end do I
...
//AJAXIT: retrieve some ajax results and stick 'em in at the top of the main
// pane. gleefully stolen straight from KoL itself (with some drastic
// simplifications).
...
function AddTopLink(putWhere, target, href, html, space) {
  ...
  $(putWhere).append(a);
  if (space) {
    $(putWhere).append(document.createTextNode(" "));
  }
  return a;
}
  ...
  switch (integer(itemNum)) {
    ...
    case  602:                                 // Degrassi Knoll shopping list
      if (GetCharData("friendlyknoll") == true) {
        ...
        addWhere.append(AppendLink('[Knoll]', to_place('knoll_hostile')));
      }
      break;
...
//      addWhere.append(AppendLink('[use map]',inv_use(2054))); break;
    ...
    case 2277:                                 // Fernswarthy's key
      if (place == 'adventure') {
        ...
        addWhere.append(AppendLink('[go to ruins]', 'fernruin.php'));
      }
      break;
...
//              '&quantity=1'));
    ...
    case 7179:
    case 7182:
    case 7184:                                      // First Pizza, Stankara
                                                    // Stone, Shield of Brook
    case 7181:
    case 7183:
    case 7180:
...
function RightClickHP(e) {
  ...
  if (json != undefined && json != "") {
    ...
    if (heal < 20) {
      order = ['3009', '5007', '1007', '1010', '5011', '3012'];
    }
    else if (heal < 35) {
      order = ['1010', '5011', '3012', '3009', '5007', '1007'];
    }
    else if (heal < 45) {
      order = ['5011', '1010', '3012', '3009', '5007', '1007'];
    }
    else {
      order = ['3012', '5011', '1010', '3009', '5007', '1007'];
    }
...
function Defaults(revert) {
  ...
  if (revert == 0) {
    for (i = 0; i < basePrefs.length; i++) {
      if (GetPref(basePrefs[i][0]) == undefined) {
        SetPref(basePrefs[i][0], basePrefs[i][1])
      }
    }
    for (i = 0; i < menu1.length; i++) {
      if (GetPref("menu1link" + i) == undefined) {
        SetPref("menu1link" + i, menu1[i])
      }
    }
    for (i = 0; i < menu2.length; i++) {
      if (GetPref("menu2link" + i) == undefined) {
...
function AddTopOption(name, url, select, putBefore) {
  ...
  if (putBefore == 0) {
    ...
    select.insertBefore(option, putBefore);
  }
}

// MAKEOPTION: Does what it says. Yup.
// text == label for the option.  (human-readable description)
// num = number of options to make.  Negative -> create a text input box
// instead of listbox. -1: create straight up input box. -2: create 2 input
// boxes, first one as label, second as value. pref = name of GM flag that this
// option is setting. opt1, opt2 = labels of first 2 options to create.
// (options beyond 2 must be created manually after calling this routine.)
function MakeOption(text, num, pref, opt1, opt2) {
  ...
  if (num == -2) {
    td.innerHTML = "<input style='font-size:11px;width:70px;' name=" + pref +
        "tag maxlength=16 type=text class=text value=" + text + ">";
  }
  else {
    td.innerHTML = "<span style='font-size:12px;padding-right:3px;'>" +
  ...
  if (num < 0) {
    ...
    if (num == -2) {
      ...
      if (preflink != undefined) {
        select.setAttribute('value', preflink);
      }
      else {
      ...
      select.setAttribute('value', prefVal);
    }
  }
  else {
    ...
    for (var i = 0; i < num; i++) {
      ...
      select.appendChild(option);
      if (i == 0 && opt1 != 0) {
        option.innerHTML = opt1;
      }
      if (i == 1 && opt2 != 0) {
        option.innerHTML = opt2;
      }
    }
  }
...
// ADDTOTOPOFMAIN: insert an element at the top of the main frame, but under
// the combat bar if present. function yoinked from JHunz's island management
// thingy
...
function InlineItemDescriptions() {
  ...
  document.addEventListener("click", function (e) {
    ...
    if ((e.target.tagName === "IMG") ||
      ...
      if (e.shiftKey || e.altKey || e.metaKey || e.button === 2) {
        return true;
      }
...
// n.b. game.php is the outermost, non-frame window that contains all the
// frames. as such, the script only sees it exactly once, when you're logging
// in.
function at_game() {
...
// code for NS Lair spoilers borrowed shamelessly from Tard's NS Trainer v0.8
  // monster name:[preferred combat item, funkslinging item, is this
  // lair-spoilery, special treatment flag] special treatment: 0=nothing; 1=any
  // gremlin; 2=non-tool gremlin; 3=hidden city; 4=pirate insults.
  var MonsterArray = {
...
// PART 1: FIRST-ROUND STUFF
  if (infight != "Y") {  // first time through this particular fight?
    ...
    if (monsterItem != undefined) {  // let's do something specific with this critter.
      ...
      // n.b. we set this in a separate long-term variable so that we can tweak
      // it mid-fight if needed.
  ...
  if (GetCharData("special") != 0) {  // in a fight with something special?
    switch (GetCharData("special")) {
      ...
      case 2: // gremlins that we know don't have the tool:
        ...
        break;
        //case 3 used to be for the hidden city when we needed to figure out
        // what sphere was for what altar.
...
function ProcessNunMeatDrops() {
  ...
  if ((meatSoFar == undefined) || isNaN(meatSoFar)) {
    meatSoFar = 0;
  }
  meatSoFar += meat;
...
function link_cellar(square) {
  var thissq = square.match(/(\d+)/)[1]; // get number from
                                         // "cellar.php?action=explore&whichspot=19"
  ...
  if (check(thissq, UP)) {
    $('#up > a').replaceWith(cLink(thissq, UP, "Up"));
  }
  if (check(thissq, DOWN)) {
    $('#down > a').replaceWith(cLink(thissq, DOWN, "Down"));
  }
  if (check(thissq, LEFT)) {
    $('#left > a').replaceWith(cLink(thissq, LEFT, "Left"));
  }
  if (check(thissq, RIGHT)) {
    $('#right > a').replaceWith(cLink(thissq, RIGHT, "Right"));
  }
...
// CELLAR: track what square we clicked on in order to provide exploration
// links later.
function at_cellar() {
...
// ADVENTURE: provide "Explore next square" link when we hit a non-combat in
// the Hidden City. Also provide extra functionality for certain other
// noncombats.
  ...
  switch (subloc) {
    ...
    case "?place=scg":
          ...
          (tdtext.indexOf("beat your Nemesis yet, eh?") != -1))        // Mox inter
      {
        ...
        $('p:last').append(AppendLink('[equip fledges]', equip({
          i: 3033,
          s: 3
        })))
...
function at_choice() {
  ...
  var choicetext = $('body').text(); // for finding stuff that's not in a <p>
                                     // tag.  sigh.
  ...
  if (p.length) {
    ...
    if (p0text.indexOf("actually a book.") != -1) {  // The Oracle
      ...
      for (var i = 0; i < kText.length; i++) {
        if (p0text.indexOf(kText[i][0]) != -1) {
          SetCharData("Krakrox", kText[i][1]);
        }
      }
    }
  }
...
function at_forestvillage() {
  ...
  if (plunger) {
    $('b:contains("Untinker")').append(AppendLink('[innabox]', linkloc));
  }
}
  ...
  if (document.location.search.indexOf("artist") != -1) {
    $('p').each(function () {
      ...
      if (txt.indexOf('Knob Goblin') != -1) {
        p.append(AppendLink('[Knob outskirts (1)]', snarfblat(114)));
      }
      if (txt.indexOf('Haunted Pantry') != -1) {
        p.append(AppendLink('[Pantry (1)]', snarfblat(113)));
      }
      if (txt.indexOf('Back Alley') != -1) {
        p.append(AppendLink('[Alley (1)]', snarfblat(112)));
      }
    });
  }
}
...
function at_bounty() {
  ...
  $('img[width=30][class!=hand]').each(function (n, el) {
    var $img = $(this);
    if ($img.attr("title") === "filthy lucre") {
      return true;
    }
...
*/
  ...
  // the second <table>. going back to see the BHH subsequently also gives the
  // text in the first <td>, but that <td> also encompasses the rest of the
  // form.
...
function at_beerpong() {
  ...
  if (html) {
    if (html.indexOf('ll flay') != -1) {
      val = 1;
    }
    else if (html.indexOf('craven') != -1) {
      val = 2;
    }
    else if (html.indexOf('pestilent') != -1) {
      val = 3;
    }
    else if (html.indexOf('run red') != -1) {
      val = 4;
    }
    else if (html.indexOf('ned goat') != -1) {
      val = 5;
    }
    else if (html.indexOf('tle girl') != -1) {
      val = 6;
    }
    else if (html.indexOf('some worm') != -1) {
      val = 7;
    }
    else if (html.indexOf('ngle man') != -1) {
      val = 8;
    }
...
function at_inventory() {
  ...
  if (searchString.indexOf("which=2") != -1) {
    gearpage = 1;
...
// GetPref('backup'),
                    ...
                    for (var i = 0, len = document.links.length; i < len; i++) {
                      if (document.links[i].text.indexOf("...") != -1) {
                        if (response.indexOf("custom outfits") == -1) {
                          ...
                          document.links[i].innerHTML = "[too many outfits]";
                        }
                        break;
                      }
                    }
          ...
          if (processingUnequipAll == 1) {
            unlink.parentNode.insertBefore(
...
// this is where we go back to a useful location if we've done/used something
// elsewhere that caused the inventory page to load.
...
// and this is where we add all the nifty little links after equipping
// something.
...
function at_palindome() {
  if ($('body').text().indexOf("accessible to you") != -1) {
    $('td:contains("That place"):last')
        .append(AppendLink('[equip Talisman, o Nam, silat! pi uqe?]', equip({
          i: 486,
          s: 3
        })));
  }
...
function at_bigisland() {
  ...
  // if we're showing the junkyard, add onclick events to track which junkyard
  // zone we go into.
...
function at_store() {
  ...
  if (insput.length > 0) {
    ...
    if (insput.length > 0) {
      ...
      whichstore = document.location.search.match(/whichstore\=([a-z0-9])/)[1];
    }
  }
  ...
  // right-click on the image of the shopkeeper to put on your travoltan
  // trousers without leaving the store.
  ...
  if (GetPref('shortlinks') > 1 && firstTable != undefined &&
    ...
    if (descId != undefined) {
      ...
      //if ((whut == 'skill' || whut == 'use') &&
      // firstTable.children('tr:eq(1)').text().indexOf("an item:") == -1)
  ...
  if (GetPref('shortlinks') > 1) {
    if (whichstore == 'h') {
      if (noform == 1) {
        ...
        swap = AppendOutfitSwap(0, "Whoa, Clear Your Head, Man", 0);
      }
      $('p:first').append(swap);
    }
    else if (whichstore == 'b') {
      if (noform == 1) {
        swap = AppendOutfitSwap(1, "Wave Your Hand And Say \"But I Am A Bugbear.\"", 0);
      }
      else {
        swap = AppendOutfitSwap(0, "Sneak Away Before The Bugbear Catches On", 0);
      }
      $('p:first').append(swap);
    }
  }
...
function at_craft() {
  ...
  // buy from the Bad Moon store if it's available, since the stuff is
  // half-price there.
  ...
  if (GetPref('shortlinks') > 1) {
    ...
    if (tr.next().text().indexOf("You acquire") != -1) {
      ...
      if (bText.text().indexOf("ten-leaf clovers") != -1) {
        ...
        AddLinks(descId, bText.parent().parent().get(0), p, thePath);
      }
    }
  }
}
    ...
    $('p').each(function () {
              ...
              .attr('href', 'javascript:void(0);').click(function () {
                GM_get(server + equip({
                  ...
                  oname: GetPref('backup')
                    ...
                    function (response) {
                      parent.frames[2].location = 'http://' + server + equip({
                        a: 'outfit',
                        oid: 5
                      });
                    });
      ...
      if (txt.indexOf("invaded!") != -1 || txt.indexOf("pirates") != -1) {
        ...
        if (txt.indexOf("sky") != -1) {
          ...
          p.append(AppendLink('[beanstalk]', to_place('beanstalk')));
        }
      }
    ...
    $('b').each(function () {
      ...
      switch (txt) {
        ...
        case "The Final Ultimate Epic Final Conflict":
          ...
          break;
// can't process these here, we're doing full-string matching and we don't have
// %playername and %familiarname available. case "and the Quest for the Holy
// MacGuffin": case "Go To White Citadel":
        ...
        case "The Pretty Good Escape":
          var subtext = $(this).parent().text();
          if (subtext.indexOf("level 1")) {
            b.append(AppendLink("[level 1 (1)]", snarfblat(51)));
          }
          else if (subtext.indexOf("Level 2")) {
            b.append(AppendLink("[level 2 (1)]", snarfblat(52)));
          }
          else if (subtext.indexOf("Level 3")) {
            b.append(AppendLink("[level 3 (1)]", snarfblat(53)));
          }
          else if (subtext.indexOf("ject 37")) {
            b.append(AppendLink("[subject 37]", "cobbsknob.php?level=3&action=cell37"));
          }
          break;
...
function CompressThrallAndFamiliar(compressLevel) {
  ...
  if (compressLevel == 2) {   //show weight and XP only
    $fp2.siblings('font').find('table').appendTo($fp1.parents('p:first')); //entire
                                                                           // XP
                                                                           // table,
                                                                           // if
                                                                           // present
    ...
    $fp2.hide();
    if (reanimator) {
      $fp1.append(AppendLink('chat!', chatlink));
    }
  }
  else {                    //show nothing
    $fp2.parent().hide();
...
//if clancy is the first image, we're in compact mode, but he's 100 pixels
// tall, the little bastard.
...
//  SetData("charname",bText[0].textContent);
  ...
  if (compactMode) {
    ...
    if (isNaN(advcount)) {
      advcount = integer(data[0]);
    } // data[0] is adv if no MP is on display (e.g. ZombieMaster)
    ...
    if (GetPref('backup')) {
      ...
      img.addEventListener('contextmenu', function (event) {
        GM_get(server + 'inv_equip.php?action=customoutfit&which=2&outfitname=' +
          ...
          var msg;
          if (response.indexOf("custom outfits") == -1) {
            msg = "Outfit Backed Up";
          }
          else {
            msg = "Too Many Outfits";
          }
    ...
    if (bEff.length > 0) {
      bEff.get(0).setAttribute("oncontextmenu",
  ...
  if ($('#nudgeblock div:contains("none")').length > 0) {
    $('#nudgeblock').hide();
  }
...
//  $('#nudges').hide();
  ...
  if (temphydr) {
    ...
    if (advcount < temphydr) {
      SetCharData('hydrate', false);
    }
    else if (advcount == temphydr) {
      if (compactMode) {
        $('a[href="adventure.php?snarfblat=364"]')
  ...
  for (i = 0, len = imgs.length; i < len; i++) {
    ...
    if (imgClick == null || imgClick.substr(0, 4) != "eff(") {
      continue;
    }
    ...
    else // if (img.getAttribute('oncontextmenu') == null)
    {
      ...
      // Effect descIDs are 32 characters?? Bah, I'm not using strings that
      // long. Six characters will do.
      ...
      switch (effNum) {
        ...
        case 221: // chalk: right-click to use more
          ...
          if (img.getAttribute('oncontextmenu') == null) {
...
function at_skills() {
  ...
  if (miniSkills) {
    ...
    for (var i = 0, len = sel.childNodes.length; i < len; i++) {
      ...
      //s.setAttribute('style','max-width:180px;'); //this doesn't work
      // anymore???
      var temp = s.value;
...
function at_charsheet() {
  ...
  // see if the character qualifies for the Myst-Class innate 5% resistance
  // bonus...
  var mystBonus = 0;
  var HPMPstats = $("table table table td:lt(10)").text();
  if (HPMPstats.indexOf("Mana Points") != -1) {
    mystBonus = 5;
  }
  ...
  $('td:contains("Protection"):not(:has(table))').each(function () {
    ...
    if (ProtNum == 0) {  // it wasn't any of the easy ones
      ...
      if (pstring.indexOf("Really") != -1) {
        ProtNum += 1;
      }
      if (pstring.indexOf("Extremely") != -1) {
        ProtNum += 3;
      }
      // exclusive modifiers
      if (pstring.indexOf("Amazingly") != -1) {
        ProtNum += 6;
      }
      else if (pstring.indexOf("Extraordinarily") != -1) {
        ProtNum += 12;
      }
      else if (pstring.indexOf("Incredibly") != -1) {
        ProtNum += 18;
      }
      else if (pstring.indexOf("Staggeringly") != -1) {
        ProtNum += 24;
      }
      else if (pstring.indexOf("Mind-bogglingly") != -1) {
        ProtNum += 30;
      }
      else if (pstring.indexOf("Inconceivably") != -1) {
        ProtNum += 36;
      }
      else if (pstring.indexOf("Unthinkably") != -1) {
        ProtNum += 42;
      }
      else if (pstring.indexOf("Indescribably") != -1) {
        ProtNum += 48;
      }
...
function at_pyramid() {
  ...
  checkInv.addEventListener('click', function (evt) {
    ...
    GM_get(server + 'api.php?what=inventory&for=MrScript', function (response) {
      ...
      if (html == "You have ") {
        html = "Nothing special in inventory. (Sorry.)";
      }
...
// pyramid de-ratter cheerfully swiped from JiK4Eva
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    // must match the href at the start of the TD element only, or this will
    // over-match and step on our ratchet/inv links above.
  ...
  if (document.location.search == "?action=gates") {
    for (var i = 0; i < 3; i++) {
      ...
      if (ptxt.indexOf("Suc Rose") != -1) {
        p.appendChild(AppendLink('[sugar rush]', inv_use(540)));
      }
      else if (ptxt.indexOf("Hilarity") != -1) {
        p.appendChild(AppendLink('[gremlin juice]', inv_use(2631)));
      }
      else if (ptxt.indexOf("Humility") != -1) {
        p.appendChild(AppendLink('[wussiness]', inv_use(469)));
      }
      else if (ptxt.indexOf("Morose Morbidity") != -1) {
        p.appendChild(AppendLink('[thin black candle]', inv_use(620)));
      }
      else if (ptxt.indexOf("Slack") != -1) {
        p.appendChild(AppendLink('[mick\'s IVH rub]', inv_use(618)));
      }
      else if (ptxt.indexOf("Spirit") != -1) {
        p.appendChild(AppendLink('[pygmy pygment]', inv_use(2242)));
      }
      else if (ptxt.indexOf("Porcupine") != -1) {
        p.appendChild(AppendLink('[super-spiky hair gel]', inv_use(587)));
      }
      else if (ptxt.indexOf("Viper") != -1) {
        p.appendChild(AppendLink('[adder bladder]', inv_use(2056)));
      }
      else if (ptxt.indexOf("Locked Gate") != -1) {
        p.appendChild(AppendLink('[Black No. 2]', inv_use(2059)));
      }// gate 2:
      else if (ptxt.indexOf("Machismo") != -1) {
        p.appendChild(AppendLink('[meleegra]', inv_use(1158)));
      }
      else if (ptxt.indexOf("Flame") != -1) {
        p.appendChild(AppendLink('[jabanero gum]', inv_use(300)));
      }
      else if (ptxt.indexOf("Intrigue") != -1) {
        p.appendChild(AppendLink('[handsomeness]', inv_use(1162)));
      }
      else if (ptxt.indexOf("Mystery") != -1) {
        p.appendChild(AppendLink('[pickle gum]', inv_use(299)));
      }
      else if (ptxt.indexOf("the Dead") != -1) {
        p.appendChild(AppendLink('[marzipan skull]', inv_use(1163)));
      }
      else if (ptxt.indexOf("Torment") != -1) {
        p.appendChild(AppendLink('[tamarind gum]', inv_use(297)));
      }
      else if (ptxt.indexOf("Zest") != -1) {
        p.appendChild(AppendLink('[lime & chile gum]', inv_use(298)));
      }// gate 3:
      else if (ptxt.indexOf("Hidden") != -1) {
        p.appendChild(AppendLink('[dod potion - object]', 'multiuse.php', false));
      }
      else if (ptxt.indexOf("Light") != -1) {
        p.appendChild(AppendLink('[dod potion - moxie]', 'multiuse.php', false));
      }
      else if (ptxt.indexOf("Mind") != -1) {
        p.appendChild(AppendLink('[dod potion - myst]', 'multiuse.php', false));
      }
      else if (ptxt.indexOf("Ogre") != -1) {
        p.appendChild(AppendLink('[dod potion - muscle]', 'multiuse.php', false));
      }
      else if (ptxt.indexOf("Not a Gate") != -1) {
        p.appendChild(AppendLink('[dod potion - teleport]', 'multiuse.php', false));
      }
    }
...
// door and familiar coding shamelessly borrowed from Tard's NS Trainer script
// v0.8
  if (window.location.search == "" && GetPref("lairspoil") == 1) {
    ...
    if (aP[1] && aP[1].innerHTML.indexOf("BEWARE") != -1) {
      ...
      if (code && document.getElementsByTagName("input")[1]) {
        document.getElementsByTagName("input")[1].value = code;
      }
    }
  }
    ...
    window.addEventListener("load", function (e) {
      ...
      with (newDiv) {
        id = "nsToolsHead";
        innerHTML = "Naughty Sorceress Tools:";
      }
      with (newDiv.style) {
        background = "blue";
        textAlign = "center";
      ...
      if (famId) {
        oDiv.innerHTML = '<a href="familiar.php?action=newfam&newfam=' + famId + '">Use the ' + famName + ', Luke!</a>';
      }
    }, false);
  }
}
...
// Image courtesy of Picklish's Mining Helper script.
  ...
  if (document.location.search.indexOf("mine=1") != -1) {  // don't display quest data in the knob shaft (mine=2), only the dwarf mine
    ...
    if (oretype != '') {
      ...
      if (foundamount) {
        msg += " (You've found " + foundamount + " so far.)";
      }
...
// track what each square gave us:
  ...
  //also add onclick() to the "Find new cavern" button to wipe out our stored
  // values.
...
function at_campground() {
  ...
  if (GetPref("telescope")
    ...
    resultsBar.parent().next().find('p').each(function (t) {
      ...
      var snarf = false;
      var gateInfo = [
        //note that using functions to define where to go just KILLS
        // performance since the array must then be constructed on every page
        // instead of being a static, pre-defined object.  drat. identifying
        // text              item required           image           where to
        // go                                     link text               item
        // ID
...
function at_basement() {
  ...
  for (var i = 0, len = ins.length; i < len; i++) {
    ...
    if (temp.indexOf("Drunk's Drink") != -1) {
      phial = 1638;
    }
    else if (temp.indexOf("Pwn the Cone") != -1) {
      phial = 1640;
    }
    else if (temp.indexOf("Hold your nose") != -1) {
      phial = 1641;
    }
    else if (temp.indexOf("Typewriter,") != -1) {
      phial = 1637;
    }
    else if (temp.indexOf("Vamps") != -1) {
      phial = 1639;
    }
    if (phial > 0 && phial != (curphial + 1448)) { // In the biz, we call this this the Phial Phudge Phactor.
      ...
      aa.addEventListener('click', function (event) {
        ...
        if (this.getAttribute('curphial')) //remove old phial effect if present
        {
          ...
          GM_get(server + this.getAttribute('phial'), function (details) {
            var ph = document.getElementById('usephial');
            ph.innerHTML = "";
            top.frames[1].location.reload();
          });
  ...
  if (str != "") {
    bim.parentNode.innerHTML += "<br><span class='small'><b>" + str + "</b></span>";
  }
}
...
function spoil_beach() {
  $('img').each(function () {
    ...
    if (src.indexOf("newbeach9") != -1) {
      ml = '20-25';
    }
    else if (src.indexOf("desert.gif") != -1) {
      ml = '134-142';
    }
    else if (src.indexOf("oasis") != -1) {
      ml = '132-137';
    }
    if (ml) {
...
function spoil_pyramid() {
  ...
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("mid2") != -1) {
      msg = 'ML: 162-176';
    }
    else if (src.indexOf("mid3b") != -1) {
      msg = 'ML: 162-180';
    }
    else if (src.indexOf("mid4_5") != -1) {
      msg = 'Keep Going...';
    }
    else if (src.indexOf("mid4_2") != -1) {
      msg = 'Keep Going...';
    }
    else if (src.indexOf("mid4_4") != -1) {
      msg = 'Phase 1: Token';
    }
    else if (src.indexOf("mid4_3") != -1) {
      msg = 'Phase 2: ???';
    }
    else if (src.indexOf("mid4_1.") != -1) {
      msg = 'Phase 3: Profit!';
    }
    if (msg) {
...
function spoil_plains() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("knob1") != -1) {
      ml = '1-2';
    }
    else if (src.indexOf("funhouse") != -1) {
      ml = '14-20';
    }
    else if (src.indexOf("knoll1") != -1) {
      ml = '10-13';
    }
    else if (src.indexOf("cemetary") != -1) {
      ml = '18-20 / 53-59';
    }
    else if (src.indexOf("dome") != -1) {
      ml = '116-135';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
...
function spoil_cobbsknob() {
  $('area').each(function () {
    ...
    if (alt.indexOf('Outskirts') != -1) {
      ml = '1-2';
    }
    else if (alt.indexOf('Barracks') != -1) {
      ml = '22-30';
    }
    else if (alt.indexOf('Kitchens') != -1) {
      ml = '20-23';
    }
    else if (alt.indexOf('Harem') != -1) {
      ml = '25-30';
    }
    else if (alt.indexOf('Treasury') != -1) {
      ml = '21-25';
    }
    else if (alt.indexOf('Throne') != -1) {
      ml = '57\nMCD=3 for Glass Balls (+14 Mys)\nMCD=7 for Codpiece (+7 Mox)';
    }
    else if (alt.indexOf('Laboratory') != -1) {
      ml = '38-45';
    }
    else if (alt.indexOf('Knob Shaft') != -1) {
      ml = '30';
    }
    else if (alt.indexOf('Menagerie Level 1') != -1) {
      ml = '50-56';
    }
    else if (alt.indexOf('Menagerie Level 2') != -1) {
      ml = '60-66';
    }
    else if (alt.indexOf('Menagerie Level 3') != -1) {
      ml = '70-76';
    }
...
function spoil_crypt() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("ul.gif") != -1) {
      ml = '53-57, +Item';
    }// nook
    else if (src.indexOf("ur.gif") != -1) {
      ml = '56-59, olfact dirty old lihc';
    }// niche
    else if (src.indexOf("ll.gif") != -1) {
      ml = '55; +ML, +NC';
    }// cranny
    else if (src.indexOf("lr.gif") != -1) {
      ml = '54-58, +Init';
    } //alcove
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
  ...
  $('area').each(function () {
    var ml = null;
    var alt = this.getAttribute('alt');
    if (alt.indexOf("Haert of the Cyrpt") != -1) {
...
function spoil_woods() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("8bitdoor") != -1) {
      ml = '20-25';
    }
    else if (src.indexOf("kforest") != -1) {
      ml = '123-133';
    }
    else if (src.indexOf("hiddencity") != -1) {
      ml = '145-156';
    }
    else if (src.indexOf("forest") != -1) {
      ml = '5';
    }
    else if (src.indexOf("barrow") != -1) {
      ml = '56-65';
    }
    else if (src.indexOf("pen.") != -1) {
      ml = '13-20';
    }
    else if (src.indexOf("grove") != -1) {
      ml = '34-36';
    }
...
function spoil_tavern() {
  $('area').each(function () {
    ...
    if (src.indexOf("A Barroom Brawl") != -1) {
      ml = '6-10';
    }
    else if (src.indexOf("Tavern Cellar") != -1) {
      ml = '10';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
...
function spoil_island() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("island4") != -1) {
      ml = '39-41';
    }
    else if (src.indexOf("island6") != -1) {
      ml = '39-41';
    }
    else if (src.indexOf("cove") != -1) {
      ml = '61-69';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
...
function spoil_cove() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("cove3_2x1") != -1) {
      ml = '80-83';
    }
    else if (src.indexOf("cove3_3x1b") != -1) {
      ml = '100';
    }
    else if (src.indexOf("cove3_3x3b") != -1) {
      ml = '120';
    }
    else if (src.indexOf("cove3_5x2b") != -1) {
      ml = '140';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
...
function spoil_da() {    // dungeoneer's association, yay.
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf('ddoom') != -1) {
      ml = '57-61';
    }
    else if (src.indexOf('haikudungeon') != -1) {
      ml = '3-5';
    }
    else if (src.indexOf('limerickdungeon') != -1) {
      ml = 'N/A';
    }
    else if (src.indexOf('dailydungeon') != -1) {
      ml = '12-20';
    }
    else if (src.indexOf('greater.gif') != -1) {
      ml = '55-60?';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
...
function spoil_friars() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("neck") != -1) {
      ml = '40-52';
    }
    else if (src.indexOf("heart") != -1) {
      ml = '42-50';
    }
    else if (src.indexOf("elbow") != -1) {
      ml = '44-48';
    }
...
function spoil_pandamonium() {
  $('area').each(function () {
    var ml = null;
    var alt = this.getAttribute('alt');
    if (alt.indexOf('Slums') != -1) {
      ml = '40-52';
    }
    else if (alt.indexOf('Go Inside') != -1) {
      ml = '60-66; CH imp';
    }
    else if (alt.indexOf('Go Backstage') != -1) {
      ml = '55-60; serialbus';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
...
function spoil_mclargehuge() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("goatlet") != -1) {
      ml = '68';
    }
    else if (src.indexOf("lair") != -1) {
      ml = '80-90';
    }
    else if (src.indexOf("mine") != -1) {
      ml = '53-58';
    }
    else if (src.indexOf("slope") != -1) {
      ml = '73-75';
    }
    else if (src.indexOf("peak") != -1) {
      ml = '105-107';
    }
...
function spoil_bigisland() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("nunnery1") != -1) {
      ml = 'ML: 168';
    }
    else if (src.indexOf("bfleft") != -1) {
      ml = this.getAttribute('title') + ' (ML: 170-210)';
    }// don't overwrite image number info
    else if (src.indexOf("bfright") != -1) {
      ml = this.getAttribute('title') + ' (ML: 170-210)';
    }
    else if (src.indexOf("junk1") != -1) {
      ml = 'ML: 168-172';
    }
    else if (src.indexOf("junk3") != -1) {
      ml = 'ML: 168-172';
    }
    else if (src.indexOf("junk5") != -1) {
      ml = 'ML: 168-172';
    }
    else if (src.indexOf("junk7") != -1) {
      ml = 'ML: 168-172';
    }
    else if (src.indexOf("filth4") != -1) {
      ml = 'ML: 165';
    }
    else if (src.indexOf("filth6") != -1) {
      ml = 'ML: 167';
    }
    else if (src.indexOf("filth8") != -1) {
      ml = 'ML: 169';
    }
    else if (src.indexOf("filth9") != -1) {
      ml = 'ML: 173';
    }
    else if (src.indexOf("farm1d") != -1) {
      ml = 'ML: 170-179, cold (weak=hot/spooky)';
    }
    else if (src.indexOf("farm2d") != -1) {
      ml = 'ML: 170-177, hot (weak=stench/sleaze)';
    }
    else if (src.indexOf("farm3d") != -1) {
      ml = 'ML: 173, sleaze (weak=cold/spooky)';
    }
    else if (src.indexOf("farm4d") != -1) {
      ml = 'ML: 175 (no elemental alignment)';
    }
    else if (src.indexOf("farm5d") != -1) {
      ml = 'ML: 166-168';
    }
    else if (src.indexOf("farm6d") != -1) {
      ml = 'ML: 169-174, stench (weak=cold/sleaze)';
    }
    else if (src.indexOf("farm7d") != -1) {
      ml = 'ML: 171-175, spooky (weak=hot/stench)';
    }// farm8 is McMillicancuddy, he's never an adventurable zone
    else if (src.indexOf("farm9d") != -1) {
      ml = 'ML: 165-180 (no elemental alignment)';
    }
    else if (src.indexOf("bmim24") != -1) {
      ml = 'ML: 240-250';
    }// wrong section?  hippy camp, bombed.
    else if (src.indexOf("bmim23") != -1) {
      ml = 'ML: 230-255';
    }// wrong section?  frat house, bombed.
    else if (src.indexOf("lighthouse_left") != -1) {
      ml = 'ML: 171';
    }

    if (ml) {
      this.setAttribute('title', ml);
    }
  });
}
...
function spoil_postwarisland() {
  $('img').each(function () {
    ...
    else if (src.indexOf("22.gif") != -1) {
      ml = '61-69';
    }// pirate cove, undisguised
    else if (src.indexOf("23.gif") != -1) {
      ml = '230-255';
    }// frathouse, bombed
    else if (src.indexOf("24.gif") != -1) {
      ml = '240-250';
    }// hippy camp, bombed
    else if (src.indexOf("25.gif") != -1) {
      ml = '169-172';
    }// Junkyard
    else if (src.indexOf("26.gif") != -1) {
      ml = '169-180';
    }// McMillicancuddy
    else if (src.indexOf("27.gif") != -1) {
      ml = '39-41';
    }// frathouse, unbombed
...
function spoil_thesea() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("sea1") != -1) {
      ml = '300-330';
    }// briny
    else if (src.indexOf("sea2") != -1) {
      ml = '350-400';
    }// brinier
...
function spoil_seafloor() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("garden") != -1) {
      ml = '350-450';
    }// octopus's garden
    else if (src.indexOf("divebar") != -1) {
      ml = '400-600';
    }// dive bar
    else if (src.indexOf("mine") != -1) {
      ml = '400-500';
    }// anemone mine
    else if (src.indexOf("trench") != -1) {
      ml = '400-550';
    }// marinara trench
    else if (src.indexOf("utpost") != -1) {
      ml = '650-750';
    }// mer-kin outpost
    else if (src.indexOf("shipwreck") != -1) {
      ml = '400-700';
    }// Fitzsimmons
    else if (src.indexOf("reef") != -1) {
      ml = '400-500';
    }// Madness Reef
    else if (src.indexOf("abyss") != -1) {
      ml = '600-650';
    }// Calyginous Abyss
...
function spoil_sea_merkin() {
  $('area').each(function () {
    ...
    if (title.indexOf('Mer-kin Colosseum') != -1) {
      ml = '800-1300';
    }
    else if (title.indexOf('Mer-kin Gymnasium') != -1) {
      ml = '720-780';
    }
    else if (title.indexOf('Mer-kin Elementary School') != -1) {
      ml = '600-700';
    }
    else if (title.indexOf('Mer-kin Library') != -1) {
      ml = '750-850';
    }
    else if (title.indexOf('Mer-kin Temple') != -1) {
      ml = '4000 (as gladiator), 400 (as scholar), 42000 (Dad)';
    }
    if (ml) {
      this.setAttribute('title', 'ML: ' + ml);
    }
...
function spoil_sea_skatepark() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("rumble") != -1) {
      ml = "ML: 350-450";
    }
    else if (src.indexOf("ice_territory") != -1) {
      ml = "ML: 350-400";
    }
    else if (src.indexOf("roller_territory") != -1) {
      ml = "ML: 350-450";
    }
    else if (src.indexOf("lutz") != -1) {
      ml = "receive 30 turns of Fishy (underwater adventures take only 1 turn)";
    }
    else if (src.indexOf("comet") != -1) {
      ml = "receive 30 turns of Finstrong (reduce diving penalty by 30)";
    }
...
function spoil_wormwood() {
  $('img').each(function () {
    var ml = null;
    var src = this.getAttribute('src');
    if (src.indexOf("wormwood3") != -1) {
      ml = 'ML scales up to 125\nturn 9-7 for skirt, STLT, myst\nturn 5-4 for !pipe, necklace, moxie\nturn 1 for flask, mask, muscle';
    } // Mansion
    if (src.indexOf("wormwood4") != -1) {
      ml = 'ML scales up to 125\nturn 9-7 for mask, !pipe, muscle\nturn 5-4 for skirt, flask, myst\nturn 1 for STLT, necklace, moxie';
    } // dome
    if (src.indexOf("wormwood8") != -1) {
      ml = 'ML scales up to 125\nturn 9-7 for necklace, flask, moxie\nturn 5-4 for mask, STLT, muscle\nturn 1 for skirt, !pipe, myst';
    } // windmill
    if (ml) {
      this.setAttribute('title', ml);
    }
...
function at_tutorial() {
  if (location.search.indexOf("toot") == -1) {
    return;
  }
...
// ICONMENU:  maybe one day we'll do something to make this travesty of a menu
// more useful, but I don't know how right now.
...
//end moonslink
  ...
  // some housekeeping stuff that I want to make sure gets checked regularly
...
//    SetCharData("altar4",'');
  ...
  $('a').each(function (ind) {
    ...
    // Yes I know this link only applies to a handful of people. I'm doing it
    // anyway.
    if (txt == "devster panel") {
      a.html("devster");
    }
    ...
    //if ((txt == "mall") && GetPref("malllink") == 1)
    ...
    if (txt == "ship") {
      a.attr("href", to_place("bugbearship"));
    }
...
//      a.after(' <a href="forestvillage.php" target="mainpane">F-ville</a>');
  ...
  if (swordGuyURL != '' && swordGuy.length > 0) {
    ...
    if (swordGuyURL.indexOf("http://") != -1) {
      ...
      guy.setAttribute('target', 'mainpane');
    }
    ...
    swordGuy.get(0).addEventListener('contextmenu', function (event) {
      ...
      if (nuhref.indexOf("http://") != -1) {
        ...
        ln.setAttribute('target', 'mainpane');
      }
      event.preventDefault();
      event.stopPropagation();
      return false;
  ...
  if (shorttop) {
    ...
    if (front != 1) {
      toprow1.setAttribute("style", "display: none;");
    }
    ...
    if (front != 2) {
      toprow2.setAttribute("style", "display: none;");
    }
    ...
    for (var j = 0; j < 10; j++) {
      ...
      var tarjay = 'mainpane';
      if (zoiks != "") {
        if (zoiks.indexOf("http://") != -1 || zoiks.indexOf("searchplayer") != -1) {
          tarjay = '_blank';
        }
...
// if splitquest pref is set, make Quests go to Current Quests and add a Notes
// entry. otherwise Quests always goes to the Notes page.
...
function buildPrefs() {
  ...
  if (!document.querySelector('#scripts')) {
    ...
    a.addEventListener('click', function (e) {
      //make our new tab active when clicked, clear out the #guts div and add
      // our settings to it
      e.stopPropagation();
      ...
      //document.querySelector('#' + scriptID).addEventListener('click',
...
// "Off", "On"));
  ...
  function createMenu1(menu1Span) {
    ...
    select.addEventListener('click', function (event) {
      event.stopPropagation();
      event.preventDefault();
      if (confirm("Restore default menu options? (Just double-checking.)") == false) {
        return;
      }
      Defaults(1);
      for (var i = 0; i < 10; i++) {
...
function autoclear_added_rows() {
  $('a[href^="javascript"]').each(function () {
    var link = $(this);
    if (link.attr('href').indexOf('add') == -1) {
      return;
    }
